#!/usr/bin/env python3
"""Build Manan's THOA report as PDF + DOCX from case_study_2.md (single source of truth)."""
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
MD_PATH = ROOT / "case_study_2.md"
    ...
    for chunk in raw.split("[PAGE BREAK]"):
        for line in chunk.splitlines():
            ...
            if line.startswith("[FIGURE:"):
                ...
                blocks.append(Block("table_row", line))
            else:
...
def build_pdf(cover, body, figs) -> Path:
    ...
    while i < len(body):
        ...
        if blk.kind == "brk":
            ...
            path_str = blk.text.strip()
            if ".png" in path_str or ".jpg" in path_str:
                from pathlib import Path
                img_path = Path(path_str)
                if img_path.exists():
                    img = Image(str(img_path), width=16.0 * cm, height=16.0 * cm * 0.6)
                    img.hAlign = "CENTER"
                    story.append(img)
            else:
            ...
            if i + 1 < len(body) and body[i+1].kind == "body" and (body[i+1].text.startswith("Figure") or body[i+1].text.startswith("*Figure")):
                i += 1
                caption_text = body[i].text.replace("*", "") # strip markdown italics
                story.append(Paragraph(html_escape(caption_text), st_cap))
        elif blk.kind == "table_row":
            from reportlab.platypus import Table, TableStyle
            from reportlab.lib import colors
            # Collect all table rows
            table_data = []
            j = i
            while j < len(body) and body[j].kind == "table_row":
                row_text = body[j].text.strip("|")
                ...
                row_cells = []
                for col in cols:
                    row_cells.append(Paragraph(html_escape(col), st_body))
                table_data.append(row_cells)
                j += 1
                
            if table_data:
                # Calculate column widths (approx)
                avail_width = 16 * cm
                num_cols = len(table_data[0])
                col_widths = [avail_width / num_cols] * num_cols
                
                # Make the first column slightly wider if its 4 cols
                if num_cols == 4:
                    col_widths = [avail_width * 0.25, avail_width * 0.35, avail_width * 0.20, avail_width * 0.20]
                elif num_cols == 3:
                    col_widths = [avail_width * 0.2, avail_width * 0.4, avail_width * 0.4]
